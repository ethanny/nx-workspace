...
describe('SignUpUserHandler', () => {
    ...
    it('should sign up user and send a message to SNS if processSNSqueue is true', async () => {
        const command = new SignUpUserCommand('test@example.com', 'password', true);
        ...
        expect(authenticationDatabaseService.createInitialRecord).toHaveBeenCalledWith('test@example.com', AuthenticationAction.SIGN_UP_USER);
    ...
    it('should throw BadRequestException if an error occurs', async () => {
        const command = new SignUpUserCommand('test@example.com', 'password', false);
