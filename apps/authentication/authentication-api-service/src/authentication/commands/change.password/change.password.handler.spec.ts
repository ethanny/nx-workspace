...
describe('ChangePasswordHandler', () => {
    ...
    it('should change password and send a message to SNS if processSNSqueue is true', async () => {
        const command = new ChangePasswordCommand('test@example.com', 'newPassword', true);
        const authRequestRecord = { authRequestId: '123', email: 'test@example.com' };
        const authRequestDto: AuthRequestDto = { authRequestId: '123', recordStatus: AuthenticationRecordStatus.PENDING, email: 'test@example.com', password: 'newPassword' };
        ...
        expect(authenticationDatabaseService.createInitialRecord).toHaveBeenCalledWith('test@example.com', AuthenticationAction.CHANGE_PASSWORD);
    ...
    it('should throw BadRequestException if an error occurs', async () => {
        const command = new ChangePasswordCommand('test@example.com', 'newPassword', false);
