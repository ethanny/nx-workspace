...
describe('ResendInvitationHandler', () => {
    ...
    it('should resend invitation and send a message to SNS if processSNSqueue is true', async () => {
        const command = new ResendInvitationCommand('test@example.com', true);
        ...
        expect(authenticationDatabaseService.createInitialRecord).toHaveBeenCalledWith('test@example.com', AuthenticationAction.RESEND_INVITATION);
    ...
    it('should throw BadRequestException if an error occurs', async () => {
        const command = new ResendInvitationCommand('test@example.com', false);
