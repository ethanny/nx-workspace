...
describe('ForgotPasswordHandler', () => {
    ...
    it('should trigger forgot password and send a message to SNS if processSNSqueue is true', async () => {
        const command = new ForgotPasswordCommand('test@example.com', true);
        ...
        expect(authenticationDatabaseService.createInitialRecord).toHaveBeenCalledWith('test@example.com', AuthenticationAction.FORGOT_PASSWORD);
    ...
    it('should throw BadRequestException if an error occurs', async () => {
        const command = new ForgotPasswordCommand('test@example.com', false);
