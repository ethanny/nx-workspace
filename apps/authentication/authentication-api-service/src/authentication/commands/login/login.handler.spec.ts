...
describe('LoginHandler', () => {
    ...
    it('should log in user and send a message to SNS if processSNSqueue is true', async () => {
        const command = new LoginCommand('test@example.com', 'password', true);
        const authRequestRecord = { authRequestId: '123', email: 'test@example.com' };
        const authRequestDto: AuthRequestDto = { authRequestId: '123', recordStatus: AuthenticationRecordStatus.PENDING, email: 'test@example.com', password: 'password' };
        ...
        expect(authenticationDatabaseService.createInitialRecord).toHaveBeenCalledWith('test@example.com', AuthenticationAction.LOGIN);
    ...
    it('should throw BadRequestException if an error occurs', async () => {
        const command = new LoginCommand('test@example.com', 'password', false);
