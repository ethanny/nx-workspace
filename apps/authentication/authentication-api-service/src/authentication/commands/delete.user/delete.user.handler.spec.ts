...
describe('DeleteUserHandler', () => {
    ...
    it('should delete user and send a message to SNS if processSNSqueue is true', async () => {
        const command = new DeleteUserCommand('test@example.com', true);
        ...
        expect(authenticationDatabaseService.createInitialRecord).toHaveBeenCalledWith('test@example.com', AuthenticationAction.DELETE_COGNITO_USER);
    ...
    it('should throw BadRequestException if an error occurs', async () => {
        const command = new DeleteUserCommand('test@example.com', false);
