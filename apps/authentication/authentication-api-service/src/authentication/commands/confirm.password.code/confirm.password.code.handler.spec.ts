...
describe('ConfirmPasswordCodeHandler', () => {
    ...
    it('should confirm password code and send a message to SNS if processSNSqueue is true', async () => {
        const command = new ConfirmPasswordCodeCommand('test@example.com', 'password', 'code', true);
        const authRequestRecord = { authRequestId: '123', email: 'test@example.com' };
        const authRequestDto: AuthRequestDto = { authRequestId: '123', recordStatus: AuthenticationRecordStatus.PENDING, email: 'test@example.com', password: 'password', code: 'code' };
        ...
        expect(authenticationDatabaseService.createInitialRecord).toHaveBeenCalledWith('test@example.com', AuthenticationAction.CONFIRM_PASSWORD_CODE);
    ...
    it('should throw BadRequestException if an error occurs', async () => {
        const command = new ConfirmPasswordCodeCommand('test@example.com', 'password', 'code', false);
