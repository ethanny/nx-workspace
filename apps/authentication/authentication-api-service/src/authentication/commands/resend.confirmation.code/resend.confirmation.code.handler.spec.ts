// apps/authentication/authentication-api-service/src/authentication/commands/resend.confirmation.code/resend.confirmation.code.handler.spec.ts
...
describe('ResendConfirmationCodeHandler', () => {
    ...
    it('should resend confirmation code and send a message to SNS if processSNSqueue is true', async () => {
        const command = new ResendConfirmationCodeCommand('test@example.com', true);
        ...
        expect(authenticationDatabaseService.createInitialRecord).toHaveBeenCalledWith('test@example.com', AuthenticationAction.RESEND_CONFIRMATION_CODE);
    ...
    it('should throw BadRequestException if an error occurs', async () => {
        const command = new ResendConfirmationCodeCommand('test@example.com', false);
